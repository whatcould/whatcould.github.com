...
I made minor modifications to autotest, so until (unless?) those changes get pulled into the main streams, *you'll have to use my fork* that code (or pull in my last commits to your own forks). You also may need an updated version of the Engines plugin (see below).
